...
namespace Dorofii.Barcode.Runtime
{
    ...
    public static class Barcode
    {
        ...
        class WebCameraImpl : IWebCamera
        {
            ...
            public async Task<DecodeResult> DecodeAsync(RawImage rawImage, CancellationToken cancellationToken = default)
            {
                if (_isDisposed)
                    throw new Exception("WebCamera is already disposed");
            ...
            private Texture2D ReadTexture2DFromRawImage(RawImage rawImage)
            {
                ...
                // Создаем временный RenderTexture
                RenderTexture rt = RenderTexture.GetTemporary(sourceWidth, sourceHeight, 0, RenderTextureFormat.Default);
                RenderTexture.active = rt;
